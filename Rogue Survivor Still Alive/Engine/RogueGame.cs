...
namespace djack.RogueSurvivor.Engine
{
    class RogueGame
    {
        ...
        public const int FIRE_DAMAGE = 5; //@@MP (Release 4)
        ...
        public RogueGame(IRogueUI UI)
        {
            ...
            Logger.WriteLine(Logger.Stage.INIT_MAIN, "creating SoundManager"); //@@MP (Release 2)
            switch (SetupConfig.Sound)
            {
                case SetupConfig.eSound.SOUND_MANAGED_DIRECTX:
                    ...
                    break;
                case SetupConfig.eSound.SOUND_SFML:
                    m_SFXManager = new SFMLSoundManager();
                    break;
                default:
        ...
        public void Run()
        {
            ...
            m_SFXManager.StopAll(); //@@MP (Release 2)
        ...
        void HandleHiScores(bool saveToTextfile)
        {
            ...
            if (saveToTextfile)
            {
                file.Append(String.Format("ROGUE SURVIVOR: STILL ALIVE {0}", SetupConfig.GAME_VERSION));
        ...
        void StartNewGame()
        {
            ...
            //@@MP - tweaked opening sequence (Release 2)
            RefreshPlayer();
        ...
        void HandleCredits()
        {
            ...
            m_UI.UI_DrawStringBold(Color.White, "'Staying Alive' modifications", left, gy);
            gy += BOLD_LINE_SPACING;
            m_UI.UI_DrawString(Color.White, @"Programming", left, gy); m_UI.UI_DrawString(Color.White, @"- MP (http://roguesurvivor.proboards.com/user/180)", right, gy);
            gy += 2 * BOLD_LINE_SPACING;
        ...
        void HandleOptions(bool ingame)
        {
            ...
            do
            {
                ...
                // footnote.
                DrawFootnote(Color.White, "Move cursor then left/right to change values, R to restore previous values, ESC to save and leave");
                m_UI.UI_Repaint();
                ...
                switch (key.KeyCode)
                {
                    ...
                    case Keys.Left:
                        switch ((GameOptions.IDs)list[selected])
                        {
                            ...
                            case GameOptions.IDs.UI_SHOW_CORPSES: s_Options.ShowCorpses = !s_Options.ShowCorpses; break;
        ...
        void HandleRedefineKeys()
        {
            ...
            do
            {
                ...
                const int O_LEAD = 33;
                const int O_LOAD = 34;
                const int O_MAKE_MOLOTOV = 50; //@@MP (Release 4)
                ...
                switch (key.KeyCode)
                {
                    ...
                    case Keys.Enter: // rebind
                        ...
                        switch (selected)
                        {
                            ...
                            case O_MAKE_MOLOTOV: command = PlayerCommand.MAKE_MOLOTOV; break; //@@MP (Release 4)
        ...
        void NextMapTurn(Map map, SimFlags sim)
        {
            ...
            if (!isLoDetail)
            {
                ...
                foreach (Actor actor in map.Actors)
                {
                    ...
                    {
                        // sanity loss.
                        if (s_Options.IsSanityEnabled) //@@MP this is where the each-turn sanity loss happens (Release 1)
                        {
                            if (--actor.Sanity <= 0) actor.Sanity = 0;
                        }
                    }
        ...
        void HandlePlayerActor(Actor player)
        {
            ...
            do
            {
                ...
                {
                    string advisorKeyBindingString; //@@MP - make the shortcut formatted properly, as the original is confusing (Release 2)
                    if (s_KeyBindings.Get(PlayerCommand.ADVISOR).ToString() == "H, Shift")
                        advisorKeyBindingString = "HINT AVAILABLE PRESS <Shift+H>";
                    else
                        advisorKeyBindingString = String.Format("HINT AVAILABLE PRESS <{0}>", s_KeyBindings.Get(PlayerCommand.ADVISOR).ToString());

                    AddOverlay(new OverlayPopup(
                        new string[] { advisorKeyBindingString },
                ...
                if (hasKey)
                {
                    ...
                    else
                    {
                        switch (command)
                        {
                            ...
                            case PlayerCommand.MAKE_MOLOTOV: //@@MP (Release 4)
                                if (TryPlayerInsanity())
                                {
                                    loop = false;
                                    break;
        ...
        bool OnRMBCorpse(Corpse c)
        {
            ...
            else if (!Rules.HasInfection(m_Session.GameMode)) //@@MP - because I gave the option to enable corpses in STD mode (Release 1)
            {
                AddMessage(MakeErrorMessage(String.Format("Cannot butcher corpses in STD game mode")));
                return false;
            }
            else
            {
        ...
        bool HandlePlayerReviveCorpse(Actor player, Point mousePos)
        {
            ...
            {
                AddMessage(MakeErrorMessage(String.Format("Cannot revive corpses in STD game mode")));
                return false;
            }
        ...
        bool HandlePlayerMakeMolotov(Actor player, Point mousePos) //@@MP (Release 4)
        {
            ...
            if (noliquor)
            {
                AddMessage(new Message("You have no suitable liquor items to make a molotov.", m_Session.WorldTime.TurnCounter, Color.Red));
                return false;
            }
            ...
            else // get choice.
            {
                ...
                else if (IsItemLiquorForMolotov(it))
                {
                    ...
                    for (int i = 0; i != liquorQuantity; i++)
                    {
                        ...
                        m_Player.Inventory.AddAll(molotov); //replace with molotovs
                    }
                    AddMessage(MakeMessage(player, String.Format(" crafted a molotov from liquor.")));
                    ClearOverlays();
                    RedrawPlayScreen();
                    return true;
                }
        ...
        bool IsItemLiquorForMolotov(Item item) //@@MP - checks if item is a liquor of a suitable type for a molotov (Release 4)
        {
            if (item == null)
                return false;
        ...
        void GetAdvisorHintText(AdvisorHint hint, out string title, out string[] body)
        {
            switch (hint)
            {
                ...
                case AdvisorHint.CITY_INFORMATION: //commented out
                ...
                case AdvisorHint.CORPSE_BUTCHER: //living
                ...
                case AdvisorHint.CORPSE_DRAG_MOVE: //commented out
                    title = "DRAGGING CORPSES";
                    body = new string[] {
                ...
                case AdvisorHint.DOORWINDOW_CLOSE: //commented out
                ...
                case AdvisorHint.MAKE_MOLOTOV: //@@MP (Release 4)
                    ...
                    body = new string[] {
                            "You've picked up some hard liquor, which can be used",
                            "to make a molotov when hovering over it with the mouse.",
        ...
        string[] DescribeCorpseLong(Corpse c, bool isInPlayerTile)
        {
            ...
            if (Rules.HasInfection(m_Session.GameMode)) //@MP - because I gave the option to enable corpses in STD mode
            {
                // 4. Medic info.
        ...
        string[] DescribeItemLong(Item it, bool isPlayerInventory)
        {
            ...
            else if (it is ItemMedicine)
            {
                ...
                if ((it.Model.ID == (int)GameItems.IDs.MEDICINE_ALCOHOL_LIQUOR_AMBER) || (it.Model.ID == (int)GameItems.IDs.MEDICINE_ALCOHOL_LIQUOR_CLEAR))
                    additionalDescription = String.Format("make molotov : <{0}>.", s_KeyBindings.Get(PlayerCommand.MAKE_MOLOTOV).ToString());
            }
        ...
        public void DoMoveActor(Actor actor, Location newLocation)
        {
            ...
            if (isJump)
            {
                ...
                if (IsAudibleToPlayer(newLocation, Rules.QUIET_NOISE_RADIUS)) //@@MP - switched to IsVisible (Release 2), changed it back to audible now that it can be supplied a range (Release 5-4)
                {
                    ...
                    {
                        if (actor.IsPlayer)
                            m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.CLIMB_FENCE_PLAYER);
                        else
                            m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.CLIMB_FENCE_NEARBY);
                    }
                    else if (mapObj.TheName == "the wrecked car")
                    {
                        if (actor.IsPlayer)    
                            m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.CLIMB_CAR_PLAYER);
                        else
                            m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.CLIMB_CAR_NEARBY);
                    }
                }
                ...
                actor.Model.Abilities.CanTalk)
            {
                ...
                if (m_Rules.RollChance(PLAYER_HEAR_SCREAMS_CHANCE) && IsAudibleToPlayer(actor.Location, Rules.QUIET_NOISE_RADIUS)) //@@MP (Release 2), changed it to audible now that it can be supplied a range (Release 5-4)
                {
                    switch (screamSelected) //@@MP (Release 2)
                    {
                ...
                else if (m_Rules.RollChance(PLAYER_HEAR_SCREAMS_CHANCE) && IsAudibleToPlayer(actor.Location, Rules.MODERATE_NOISE_RADIUS)) //@@MP - people scream a lot, so I can see why it only had a 10% chance (n/a). Changed it to audible now that it can be supplied a range (Release 5-4)
                {
                    switch (screamSelected) //@@MP (Release 2)
                    {
        ...
        void DoTriggerTrap(ItemTrap trap, Map map, Point pos, Actor victim, MapObject mobj)
        {
            ...
            else if (victim != null && IsAudibleToPlayer(location,Rules.QUIET_NOISE_RADIUS)) //@@MP - changed from visible to audible now that it can be supplied a range (Release 5-4)
            {
                ...
                if (actorScream)
                {
                    ...
                    if (!IsVisibleToPlayer(victim)) //@@MP (Release 5-4)
                        AddMessageIfAudibleForPlayer(victim.Location, MakePlayerCentricMessage("You hear screams of terror from someone", victim.Location.Position));
                }
            }
            else if (victim != null && IsAudibleToPlayer(location, Rules.MODERATE_NOISE_RADIUS)) //@@MP - changed from IsAudibleToPlayer(victim.Location)) to audible with a supplied a range (Release 5-4)
            {
                bool actorScream = false;
                switch (trap.TheName.ToString())
                {
        ...
        public bool DoPlayerBump(Actor player, Direction direction)
        {
            ...
            if (door != null && !door.IsBarricaded && door.AName == "a locked door" && !player.Model.Abilities.IsUndead) //@@MP - moved this check from Engine\Rules : CanActorGetItemFromContainer() (Release 5-3)
            {
        ...
        public void DoSingleRangedAttack(Actor attacker, Actor defender, List<Point> LoF, FireMode mode)
        {
            ...
            switch (mode)
            {
                ...
                case FireMode.RAPID:
                    ...
                    break;
                default:
                    throw new ArgumentOutOfRangeException("unhandled ranged attack fire mode");
            }
        }
        ...
        void DoSingleRangedAttack(Actor attacker, Actor defender, List<Point> LoF, float accuracyFactor)
        {
            ...
            if (IsAudibleToPlayer(attacker.Location, Rules.QUIET_NOISE_RADIUS))// || isPlayer) //@@MP - the player is shot at or shoots, so play the CQC sound (Release 2), changed from isVisible to IsAudible with a supplied a range (Release 5-4)
            {
                switch (weapon.TheName.ToString())
                {
                    ...
                    case "the Hans von Hanz pistol":
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.PISTOL_FIRE_PLAYER);
                        break;
                    case "the hunting rifle":
                    case "the precision rifle":
                    case "the army rifle":
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.HUNTING_RIFLE_FIRE_PLAYER);
                        break;
                    case "the shotgun":
                    case "the Santaman shotgun":
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.SHOTGUN_FIRE_PLAYER);
                        break;
            ...
            else if (IsAudibleToPlayer(attacker.Location, Rules.MODERATE_NOISE_RADIUS))  //@@MP - we see the shooter, so play the nearby sound (Release 2), changed from isVisible to IsAudible with a supplied a range (Release 5-4)
            {
                switch (weapon.TheName.ToString())
                {
                    ...
                    case "the army pistol":
                    case "the Hans von Hanz pistol":
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.PISTOL_FIRE_NEARBY);
                        break;
                    case "the hunting rifle":
                    case "the precision rifle":
                    case "the army rifle":
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.HUNTING_RIFLE_FIRE_NEARBY);
                        break;
                    case "the shotgun":
                    case "the Santaman shotgun":
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.SHOTGUN_FIRE_NEARBY);
                        break;
                    case "the hunting crossbow":
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.CROSSBOW_FIRE_NEARBY);
                        break;
                    default: break;
                }
            }
            else if (IsAudibleToPlayer(attacker.Location, Rules.LOUD_NOISE_RADIUS)) //&& !isPlayer && m_Rules.RollChance(PLAYER_HEAR_FIGHT_CHANCE)) //@@MP - the noise is within earshot somewhere, so play the far-off sound (Release 2), changed from isVisible to IsAudible with a supplied a range (Release 5-4)
            {
                switch (weapon.TheName.ToString())
                {
                    ...
                    case "the army pistol":
                    case "the Hans von Hanz pistol":
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.PISTOL_FIRE_FAR);
                        break;
                    case "the hunting rifle":
                    case "the precision rifle":
                    case "the army rifle":
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.HUNTING_RIFLE_FIRE_FAR);
                        break;
                    case "the shotgun":
                    case "the Santaman shotgun":
            ...
            if (m_SFXManager.IsAudioEnabled == false || !isAttVisible) //@@MP - moved out of IsAudibleToPlayer:Far if to make it applicable for all (Release 5-4)
            {
                if (m_Rules.RollChance(PLAYER_HEAR_FIGHT_CHANCE)) //@@MP - lower chance to avoid message spam
                    AddMessageIfAudibleForPlayer(attacker.Location, MakePlayerCentricMessage("You hear gun shots", attacker.Location.Position));
            }
        ...
        void DoBlast(Location location, BlastAttack blastAttack, ItemModel itemModel) //@@MP - added itemExplosiveModel to determine the relevant sfx (Release 4)
        {
            ...
            if (isVisible || IsAudibleToPlayer(location, Rules.MODERATE_NOISE_RADIUS)) //@@MP - changed from isVisible to also IsAudible with a range(Release 5 - 4)
            {
                ...
                switch (itemModel.ID) //@@MP - play the appropriate sfx for the explosive type (Release 4)
                {
                    case (int)GameItems.IDs.EXPLOSIVE_GRENADE_PRIMED:
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.GRENADE_VISIBLE);
                    ...
                    case (int)GameItems.IDs.EXPLOSIVE_MOLOTOV_PRIMED:
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.MOLOTOV_VISIBLE);
                    ...
                    case (int)GameItems.IDs.EXPLOSIVE_DYNAMITE_PRIMED:
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.DYNAMITE_VISIBLE);
                        AnimDelay(DELAY_SHORT);
                        break;
                    default: throw new ArgumentOutOfRangeException("unhandled explosive type");
                }
            ...
            else if (IsAudibleToPlayer(location, Rules.LOUD_NOISE_RADIUS)) //(m_Rules.RollChance(PLAYER_HEAR_EXPLOSION_CHANCE)) //@@MP - explosions are rare, so no need to worry about message/sfx spam (Release 2), suppiled IsAudible with a range (Release 5-4)
            {
                ...
                {
                    case (int)GameItems.IDs.EXPLOSIVE_GRENADE_PRIMED:
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.GRENADE_AUDIBLE);
                        break;
                    case (int)GameItems.IDs.EXPLOSIVE_MOLOTOV_PRIMED:
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.MOLOTOV_AUDIBLE); //@@MP - it's close enough that it would still be audible
                        break;
                    case (int)GameItems.IDs.EXPLOSIVE_DYNAMITE_PRIMED:
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.DYNAMITE_VISIBLE);
                        break;
            ...
            else if (location.Map == m_Player.Location.Map)
            {
                ...
                {
                    m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.DYNAMITE_AUDIBLE); //@@MP - it's close enough that it would still be super loud
        ...
        int ApplyExplosionDamage(Location location, int distanceFromBlast, BlastAttack blast, ItemModel itemModel) //@@MP - added ItemModel argument (Release 4)
        {
            ...
            if (itemModel.IsFlameWeapon)//(itemModel.ID == (int)GameItems.IDs.EXPLOSIVE_MOLOTOV_PRIMED) //@@MP - switched to the new method (Release 5-2)
            {
                if (m_Rules.IsWeatherRain(m_Session.World.Weather)) //@@MP - less turns duration if it's raining
                    ...
                    SetTileOnFire(map, 10, location.Position.X, location.Position.Y);
            }
            ...
            if (acceptTrade)
            {
                ...
                if (target.Leader == speaker)
                {
                    ...
                    if (isTargetReallyInterested)
                        DoSay(target, speaker, "Thanks, that's a good deal.", Sayflags.IS_FREE_ACTION);
#if false
        ...
        Item PickItemToTrade(Actor speaker, Actor buyer)
        {
            ...
            // Offer just about anything to the player.
            // Offer only interesting items to an AI.
            // Charismatic AI can force trade (//@@MP - see IsInterestingTradeItem(Actor speaker, Item offeredItem, Actor target))
            List<Item> interestedIn = null;
            foreach (Item it in inv.Items)
            {
                // make a list of items the buyer AI is interested in and the actor is willing to trade
                if (speaker.Model.Abilities.IsLawEnforcer) //@@MP - cops won't trade their gun and radio unless hungry and have no food (Release 4)
                {
                    if (it is ItemRangedWeapon || it.Model.ID == (int)GameItems.IDs.TRACKER_POLICE_RADIO)
                    {
                        if (!m_Rules.IsActorReallyHungry(speaker) || inv.HasItemOfType(typeof(ItemFood)))
                            continue; //skip this item
                    }
                }

                if ((buyer.IsPlayer) || IsInterestingTradeItem(speaker, it, buyer)) //@@MP - buyer.IsPlayer must go first, because IsInterstingTradeItem is only meant for AI and will crash the game
                {
        ...
        void OnEquipItem(Actor actor, Item it)
        {
            ...
            if (it.Model is ItemWeaponModel)
            {
                ...
                else if (it.Model is ItemRangedWeaponModel)
                {
                    ...
                    if ((actor.IsPlayer) && (it.Model != GameItems.HUNTING_CROSSBOW)) //@@MP (Release 2)
                        m_SFXManager.Play(GameSounds.EQUIP_GUN_PLAYER);
                }
            }
            #endregion
            #region Armors
            else if (it.Model is ItemBodyArmorModel)
            {
                ...
                if (actor.IsPlayer) //@@MP (Release 2)
                    m_SFXManager.Play(GameSounds.ARMOR_ZIPPER);
            }
            #endregion
            #region Batteries
            else if (it.Model is ItemLightModel)
            {
                ...
                --ltIt.Batteries;
                if (actor.IsPlayer) //@@MP (Release 2)
                    m_SFXManager.Play(GameSounds.TORCH_CLICK_PLAYER);
            }
        ...
        void DoUseMedicineItem(Actor actor, ItemMedicine med)
        {
            ...
            if (med.Model.ID == GameItems.CIGARETTES.ID) //@@MP (Release 4)
                actor.StaminaPoints = 0;
            else
            ...
            if (actor.IsPlayer) //@@MP (Release 2)
            {
                switch (med.TheName)
                {
                    ...
                    case "some cigarettes": //@@MP (Release 4)
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.SMOKING);
                        break;
        ...
        public void DoOpenDoor(Actor actor, DoorWindow door)
        {
            ...
            if (actor.IsPlayer || IsVisibleToPlayer(actor) || IsVisibleToPlayer(door))
            {
                if (door.GivesWood)
                    ...
                    m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.ROLLER_DOOR);
                else
        ...
        void PlayBashOrBreakSFX(MapObject mapObj, bool isBroken) //@@MP (Release 5-4)
        {
            ...
            if (isBroken)
            {
                if (IsVisibleToPlayer(mapObj))
                {
                    ...
                    {
                        case "glass": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BREAK_GLASSDOOR_PLAYER); break;
                        case "metal": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BREAK_METALDOOR_PLAYER); break;
                        case "chain fence": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.CLIMB_FENCE_PLAYER); break;
                        case "ceramic": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BREAK_CERAMIC_VISIBLE); break;
                        case "wood":
                        default: m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BREAK_WOODENDOOR_PLAYER); break;
                    }
                }
                else if (IsAudibleToPlayer(mapObj.Location, Rules.MODERATE_NOISE_RADIUS))
                {
                    switch (objectMaterial)
                    {
                        case "glass": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BREAK_GLASSDOOR_NEARBY); break;
                        case "metal": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BREAK_METALDOOR_NEARBY); break;
                        case "chain fence": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.CLIMB_FENCE_NEARBY); break;
                        case "ceramic": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BREAK_CERAMIC_VISIBLE); break;
                        case "wood":
                        default: m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BREAK_WOODENDOOR_NEARBY); break;
                    }
                }
            }
            else
            {
                if (IsVisibleToPlayer(mapObj))
                {
                    switch (objectMaterial)
                    {
                        case "metal": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BASH_METALDOOR_PLAYER); break;
                        case "chain fence": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.CLIMB_FENCE_PLAYER); break;
                        case "ceramic": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BASH_CERAMIC_VISIBLE); break;
                        case "wood": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BASH_WOOD_PLAYER); break;
                        case "glass":
                        default: m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BASH_OTHER_OBJECTS_PLAYER); break;
                    }
                }
                else if (IsAudibleToPlayer(mapObj.Location, Rules.MODERATE_NOISE_RADIUS))
                {
                    switch (objectMaterial)
                    {
                        case "metal": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BASH_METALDOOR_NEARBY); break;
                        case "chain fence": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.CLIMB_FENCE_NEARBY); break;
                        case "ceramic": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BASH_CERAMIC_VISIBLE); break;
                        case "wood": m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BASH_WOOD_NEARBY); break;
                        case "glass":
                        default: m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.BASH_OTHER_OBJECTS_NEARBY); break;
                    }
                }
            }
        }
        ...
        public void DoPush(Actor actor, MapObject mapObj, Point toPos)
        {
            ...
            if (IsAudibleToPlayer(mapObj.Location, Rules.QUIET_NOISE_RADIUS)) //@@MP - changed from isVisible, because if it's that close we'd hear it as loud (Release 5-4)
            {
                if (mapObj.IsMetal) //@@MP - added sound for wooden items (Release 5-4)
                    m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.PUSH_METAL_OBJECT_VISIBLE); //@@MP (Release 3)
                else
                    m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.PUSH_WOODEN_OBJECT_VISIBLE);
            ...
            else if (!isVisible)
            {
                ...
                {
                    if (mapObj.IsMetal) //@@MP - added sound for wooden items (Release 5-4)
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.PUSH_METAL_OBJECT_AUDIBLE); //@@MP (Release 3)
                    else
                        m_SFXManager.PlayIfNotAlreadyPlaying(GameSounds.PUSH_WOODEN_OBJECT_AUDIBLE);
        ...
        void InflictDamage(Actor actor, int dmg, bool splatterBlood) //@@MP - made blood splatter optional, for times were it doesn't make sense eg fires (Release 5-2)
        {
            ...
            if (splatterBlood && (!actor.Model.Abilities.IsUndead) && (actor.HitPoints > 0)) //@@MP - added check whether we even want to splatter blood (Release 5-2)
            {
                ...
                {
                    if (randomSplatterChance <= 5) //60% chance to generate a splatter
                        SplatterBlood(actor.Location.Map, actor.Location.Position); //simulations only create minor amounts of attacks, so beef up the splatter rate
                }
                else
                {*/
                switch (randomSplatterChance)
                {
                    case 0:
                    case 1: 
                    case 2:
                        SplatterBlood(actor.Location.Map, actor.Location.Position, true); //30% chance to generate a splatter
                        break;
                    default:
                        break; //do nothing
                }
        ...
        public void KillActor(Actor killer, Actor deadGuy, string reason)
        {
            ...
            if (deadGuy.IsUnique)
            {
                if (killer != null)
                    ...
                    m_Session.Scoring.AddEvent(deadGuy.Location.Map.LocalTime.TurnCounter, String.Format("* {0} died by {1}! *", deadGuy.TheName, reason));
            }
            ...
            // Blood splat and remains
            if (!deadGuy.Model.Abilities.IsUndead)
                SplatterBlood(deadGuy.Location.Map, deadGuy.Location.Position, false);

            if (s_Options.ShowCorpses || Rules.HasCorpses(m_Session.GameMode)) //@@MP - (RELEASE 1)(Release 2)
            { 
                //disabled will avoid larger saved games (//@MP according to RJ)
                if (deadGuy.Model.Abilities.IsUndead)
        ...
        public void SplatterBlood(Map map, Point position, bool ActorStillAlive = false)
        {
            ...
            switch (ActorStillAlive) //@@MP (Release 2)
            {
                case false:
                    if (map.IsWalkable(position.X, position.Y) && !tile.HasDecoration(GameImages.DECO_BLOODIED_FLOOR))
                    {
                        ...
                        //map.AddTimer(new TaskRemoveDecoration(WorldTime.TURNS_PER_DAY, position.X, position.Y, GameImages.DECO_BLOODIED_FLOOR)); //@@MP removing this adds more flavour to the game (Release 1)
                    }
                    break;
                case true:
                    if (map.IsWalkable(position.X, position.Y) && !tile.HasDecoration(GameImages.DECO_BLOODIED_FLOOR_SMALL))
                    {
                        ...
                        //map.AddTimer(new TaskRemoveDecoration(WorldTime.TURNS_PER_DAY, position.X, position.Y, GameImages.DECO_BLOODIED_FLOOR_SMALL)); //@@MP removing this adds more flavour to the game (Release 2)
                    }
                    break;
            }
            ...
            foreach (Direction d in Direction.COMPASS)
            {
                ...
                if (ActorStillAlive) //@@MP (Release 2)
                {
                    ...
                    //map.AddTimer(new TaskRemoveDecoration(WorldTime.TURNS_PER_DAY, next.X, next.Y, GameImages.DECO_BLOODIED_WALL_SMALL)); //@@MP removing this adds more flavour to the game (Release 2)
                }
                else
                {
                    ...
                    //map.AddTimer(new TaskRemoveDecoration(WorldTime.TURNS_PER_DAY, next.X, next.Y, GameImages.DECO_BLOODIED_WALL)); //@@MP removing this adds more flavour to the game (Release 1)
                }
            }
        }

        public void UndeadRemains(Map map, Point position, string thename) //@@MP - drop remains for a destroyed undead
        {
        ...
        public void DropCorpse(Actor deadGuy)
        {
            ...
            float rotation = m_Rules.Roll(30, 60); //@MP - which compass direction the body is lined to
        ...
        void HandlePlayerDecideUpgrade(Actor upgradeActor)
        {
            ...
            string youName = upgradeActor == m_Player ? "You are" : String.Concat(upgradeActor.Name, " is");
        ...
        void DoLooseRandomSkill(Actor actor) //@@MP - a typo, should be Lose
        {
        ...
        public void DrawActorSprite(Actor actor, Point screen, Color tint)
        {
            ...
            //DrawActorDecoration(actor, gx, gy, DollPart.TORSO, tint); //@@MP - not sure why it was always drawn twice... (Release 2)
        ...
        public void DrawCorpse(Corpse c, int gx, int gy, Color tint)
        {
            ...
            //DrawActorDecoration(actor, gx, gy, DollPart.TORSO, rotation, scale); //@@MP - not sure why it was always drawn twice... (Release 2)
        ...
        {
            // 1. Name & occupation
            m_UI.UI_DrawStringBold(Color.Yellow, String.Format("{0}, {1}", actor.Name, actor.Faction.MemberName), gx, gy);
        ...
        public void DrawActorSkillTable(Actor actor, int gx, int gy) //m_player, 680, 352
        {
            ...
            {
                m_UI.UI_DrawString(Color.White, String.Format("{0}-", sk.Level), x, y); //680,352. skill level-
                x += 16; //16 pixels to the right of kill level
                m_UI.UI_DrawString(Color.White, Skills.Name(sk.ID), x, y); //696,352. skill name
        ...
        bool IsInTheCoordinateRange(int coordX, int coordY, int minX, int maxX, int minY, int maxY) //@@MP - for any given coordinates, are they in a given coord range? (Release 2)
        {
        ...
        void ApplyOptions(bool ingame)
        {
            ...
            m_SFXManager.IsAudioEnabled = Options.PlaySFXs;//@@MP (Release 2)
        ...
        bool CheckCopyOfManual()
        {
            ...
            //string dst_path = GetUserDocsPath(); //@@MP - used to copy to Config/Docs for some silly reason (Release 1)
            string filename = "RS Manual.txt";
            bool copied = false;
            Logger.WriteLine(Logger.Stage.INIT_MAIN, "checking for manual...");

            if (!File.Exists(src_path + filename))
            {
                copied = true;
        ...
        string GetUserManualFilePath()
        {
            string src_path = @"Resources\Manual\";
            string filename = "RS Manual.txt";
            return String.Concat(src_path, filename);
        }
        ...
        void CheckSpecialPlayerEventsAfterAction(Actor player)
        {
            ...
            // 6. Sighting Duckman
            #region
            /*if (player != m_Session.UniqueActors.Duckman.TheActor) //@@MP - disabled as pointless (Release 2)
            {
