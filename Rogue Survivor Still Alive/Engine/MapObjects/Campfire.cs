...
{
    [Serializable]
    class Campfire : MapObject   //@@MP (Release 7-6)
    {
        #region Fields
        int m_FuelUnits;
        int m_MaxFuelUnits;
        ...
        #region Init
        public Campfire(string name, string imageID, MapObject.Break breakState, int fuelUnits)
            : base(name, imageID, breakState, Fire.BURNABLE, DoorWindow.BASE_HITPOINTS)
        {
            m_FuelUnits = fuelUnits;
            m_MaxFuelUnits = WorldTime.TURNS_PER_HOUR * 3; //90
        }
        #endregion
    }
}
