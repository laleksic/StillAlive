...
namespace djack.RogueSurvivor.Data
{
  ...
  internal class Map : ISerializable
  {
    ...
    public Point? GetExitPos(Exit exit)
    {
      if (exit == null)
        return new Point?();
      foreach (KeyValuePair<Point, Exit> exit1 in this.m_Exits)
      {
        if (exit1.Value == exit)
          return new Point?(exit1.Key);
      }
      return new Point?();
    }
    ...
    {
      foreach (Corpse corpses in this.m_CorpsesList)
      {
        if (corpses.DeadGuy == a)
        {
          this.RemoveCorpse(corpses);
          return true;
        }
      }
      return false;
    ...
    private void InsertCorpseAtPos(Corpse c)
    {
      ...
      if (this.m_aux_CorpsesByPosition.TryGetValue(c.Position, out corpseList))
        ...
        this.m_aux_CorpsesByPosition.Add(c.Position, new List<Corpse>(1)
        {
          c
        });
    }
    ...
    {
      this.m_Seed = (int) info.GetValue(nameof (m_Seed), typeof (int));
      this.m_District = (District) info.GetValue(nameof (m_District), typeof (District));
      this.m_Name = (string) info.GetValue(nameof (m_Name), typeof (string));
      this.m_LocalTime = (WorldTime) info.GetValue(nameof (m_LocalTime), typeof (WorldTime));
      this.m_Width = (int) info.GetValue(nameof (m_Width), typeof (int));
      this.m_Height = (int) info.GetValue(nameof (m_Height), typeof (int));
      this.m_Rectangle = (Rectangle) info.GetValue(nameof (m_Rectangle), typeof (Rectangle));
      this.m_Tiles = (Tile[,]) info.GetValue(nameof (m_Tiles), typeof (Tile[,]));
      this.m_Exits = (Dictionary<Point, Exit>) info.GetValue(nameof (m_Exits), typeof (Dictionary<Point, Exit>));
      this.m_Zones = (List<Zone>) info.GetValue(nameof (m_Zones), typeof (List<Zone>));
      this.m_ActorsList = (List<Actor>) info.GetValue(nameof (m_ActorsList), typeof (List<Actor>));
      this.m_MapObjectsList = (List<MapObject>) info.GetValue(nameof (m_MapObjectsList), typeof (List<MapObject>));
      this.m_GroundItemsByPosition = (Dictionary<Point, Inventory>) info.GetValue(nameof (m_GroundItemsByPosition), typeof (Dictionary<Point, Inventory>));
      this.m_CorpsesList = (List<Corpse>) info.GetValue(nameof (m_CorpsesList), typeof (List<Corpse>));
      this.m_Lighting = (Lighting) info.GetValue(nameof (m_Lighting), typeof (Lighting));
      this.m_Scents = (List<OdorScent>) info.GetValue(nameof (m_Scents), typeof (List<OdorScent>));
      this.m_Timers = (List<TimedTask>) info.GetValue(nameof (m_Timers), typeof (List<TimedTask>));
    }

    public void ReconstructAuxiliaryFields()
    {
      this.m_aux_ActorsByPosition = new Dictionary<Point, Actor>();
      foreach (Actor actors in this.m_ActorsList)
        this.m_aux_ActorsByPosition.Add(actors.Location.Position, actors);
      ...
      foreach (MapObject mapObjects in this.m_MapObjectsList)
        this.m_aux_MapObjectsByPosition.Add(mapObjects.Location.Position, mapObjects);
      this.m_aux_ScentsByPosition = new Dictionary<Point, List<OdorScent>>();
      foreach (OdorScent scent in this.m_Scents)
      {
        List<OdorScent> odorScentList;
        if (this.m_aux_ScentsByPosition.TryGetValue(scent.Position, out odorScentList))
        {
        ...
        {
          odorScentList = new List<OdorScent>() { scent };
          this.m_aux_ScentsByPosition.Add(scent.Position, odorScentList);
        }
      }
      ...
      {
        List<Corpse> corpseList;
        if (this.m_aux_CorpsesByPosition.TryGetValue(corpses.Position, out corpseList))
          corpseList.Add(corpses);
        else
          this.m_aux_CorpsesByPosition.Add(corpses.Position, new List<Corpse>(1)
          {
            corpses
          });
      }
